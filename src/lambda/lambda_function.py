...
def generate_action(reasoning_result, alarm_data):
    ...
    # Enhanced action mapping with SSM parameters
    action_map = {
        'CPUUtilization': {
            'type': 'scale_instance',
            'description': 'Scale up EC2 instances',
            'parameters': {
                'AutoScalingGroupName': 'default-asg',
                'DesiredCapacity': '3'
            }
        },
        'DatabaseConnections': {
...
def execute_action(ssm_client, action):
    ...
    try:
        ...
        execution_id = response.get('Command', {}).get('CommandId') or response.get('AutomationExecutionId')
